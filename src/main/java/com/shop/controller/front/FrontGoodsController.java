...
//        model.addAllAttributes(goodsInfo);
    ...
    public String searchGoods(@RequestParam(value = "page",defaultValue = "1") Integer pn, String keyword, Model model, HttpSession session) {
        ...
        List<Goods> goodsList = goodsService.selectGoodsListByGoodsName(keyword);
        //获取图片地址
    ...
    public String getCateGoods(Integer cateId, @RequestParam(value = "page",defaultValue = "1") Integer pn, Model model, HttpSession session) {
        User user = (User) session.getAttribute("user");

        //一页显示几个数据
        PageHelper.startPage(pn, 16);

        List<Goods> goodsList = goodsService.selectGoodsListByCateId(cateId);
        ...
        model.addAttribute("cate", category.getCatename());
        return "category";
    }
